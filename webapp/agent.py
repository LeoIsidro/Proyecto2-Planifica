    ...
    def generate_explanation(self, decisions_json):
        ...
        if self.openai_key:
            try:
                ...
                print(f"Error al conectar con OpenAI: {e}. Usando generador local.")
    ...
    def _generate_local_explanation(self, decisions):
        ...
        for d in decisions:
            ...
            if d["status"] == "accepted":
                output += "  * *Análisis del Agente:* Esta variación se alinea con el criterio estético y funcional del usuario, destacando la correcta integración del mobiliario en el espacio físico.\n"
            else:
                output += "  * *Análisis del Agente:* Se rechaza debido a incompatibilidad con el flujo de trabajo deseado, saturación visual o esquema de color inadecuado según el feedback.\n"
